...
from ..models.casestudy import CaseStudyEntry, CaseStudyQuote

from .forms import CaseStudyEntryAdminForm
...
class CaseStudyQuoteInline(admin.TabularInline):
    model = CaseStudyQuote


class CaseStudyEntryAdmin(EntryAdmin):
    form = CaseStudyEntryAdminForm
    date_hierarchy = 'creation_date'
    inlines = (CaseStudyQuoteInline,)
    ...
    fieldsets = (
        ...
        (_('Content'), {
            'fields': (('title', 'slug'), 'lead', 'content',
                       'content_right')}),
        (_('Image'), {
            'fields': ('image', 'image_caption', 'background_image')}),
        (_('Publication'), {
    ...
    search_fields = ('title', 'excerpt', 'content', 'content_right')
    ...
    def __init__(self, model, admin_site):
        ...
        super(CaseStudyEntryAdmin, self).__init__(model, admin_site)
    ...
    def make_hidden(self, request, queryset):
        ...
        """
        queryset.update(status=CaseStudyEntry.HIDDEN)
        self.message_user(
...
class CaseStudyEntryAdminCKEditorForm(CaseStudyEntryAdminForm):
    ...
    content_right = forms.CharField(label=_('Right side content'),
                                    required=False,
                                    widget=CKEditor(config_name=CONFIG_NAME))


class CaseStudyEntryAdminCKEditorMixin(object):
    form = CaseStudyEntryAdminCKEditorForm


class CaseStudyEntryCKEditorAdmin(CaseStudyEntryAdminCKEditorMixin,
                                  CaseStudyEntryAdmin):
    pass
