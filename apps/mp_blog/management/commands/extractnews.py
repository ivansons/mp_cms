...
class Command(BaseCommand):
    help = 'Extract news entries from XML file dumped from WordPress site. ' \
           'The results will be saved to json file.'
    entry_types = {'news_type'}
    ...
    def get_entries(self, xml_obj):
        ...
        for i in channel.item:
            ...
            if meta.get('news_icon'):
                news_icon = attachments[meta['news_icon']]
            else:
                news_icon = None
            entry = {
                ...
                'categories': ', '.join([
                    c.cdata for c in categories if c['domain'] == 'news_cat']),
                'tags': ', '.join([
